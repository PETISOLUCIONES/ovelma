...
def pre_init_check(cr):
    ...
    if int(''.join([str(100+int(d)) for d in  report_excel.installed_version.split('.')[2:]])) < 101103128:
        raise ValidationError('Module support "Report Designer (XLSX, XLSM)" module starting from Version 1.3.28, found Version {}. Please update the module "Report Designer (XLSX, XLSM)".'.format('.'.join(report_excel.installed_version.split('.')[2:])))
    return True