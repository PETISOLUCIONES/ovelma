...
class AccountPayment(models.Model):
    ...
    def banco_bogota(self):
        ...
        file.write(self.GetNitCompany(self.journal_id.bank_account_id.company_id.vat).ljust(11, "0"))
        file.write("002")
        ...
        for rec in self:
            ...
            file.write(rec.GetNitCompany(rec.partner_id.vat).ljust(11, "0"))
    ...
    def bancolombia(self):
        ...
        file = open(txt_path, "w")
        file.write("1")
        file.write(self.GetNitCompany(self.journal_id.bank_account_id.company_id.vat).ljust(10, "0"))
        ...
        for rec in self:
            file.write("\n")
            file.write("6")
            file.write(rec.GetNitCompany(rec.partner_id.vat).rjust(15, "0"))
    ...
    def davivienda(self):
        ...
        file = open(txt_path, "w")
        file.write("RC")
        file.write(self.GetNitCompany(self.journal_id.bank_account_id.company_id.vat).rjust(16, "0"))
        file.write("PROV")
        file.write("0000")
        ...
        for rec in self:
            file.write("\n")
            file.write("TR")
            file.write(rec.GetNitCompany(rec.partner_id.vat).rjust(16, "0"))
            file.write("".rjust(16, "0"))
            # file.write(rec.partner_id.bank_ids.acc_number.rjust(16, "0"))
            file.write(rec.partner_bank_id.acc_number.rjust(16, "0"))
            # file.write(TIPO_CUENTA_DAVIVIENDA.get(rec.partner_id.bank_ids.accountbank_type))
            file.write(TIPO_CUENTA_DAVIVIENDA.get(rec.partner_bank_id.accountbank_type))
            # file.write(rec.partner_id.bank_ids.bank_id.bic.rjust(6, "0"))
    ...
    def banco_agrario(self):
        ...
        for rec in self:
            ...
            # file.write(rec.partner_id.bank_ids.bank_id.bic.rjust(4, "0"))
            file.write(rec.partner_bank_id.bank_id.bic.rjust(4, "0"))
            file.write(rec.GetNitCompany(rec.partner_id.vat).rjust(15, " "))
    ...
    def _onchange_partner_id(self):
        if self.invoice_vendor_bill_id and self.invoice_vendor_bill_id[0].partner_bank_id:
            self.partner_bank_id = self.invoice_vendor_bill_id[0].partner_bank_id
        elif self.partner_id != self.partner_bank_id.partner_id:
            # This condition ensures we use the default value provided into
            # context for partner_bank_id properly when provided with a
            ...
            if self.partner_id and len(self.partner_id.bank_ids) > 0:
                self.partner_bank_id = self.partner_id.bank_ids.filtered(lambda m: m.bank_default == True) or self.partner_id.bank_ids[0]
            elif self.partner_id and len(self.partner_id.commercial_partner_id.bank_ids) > 0:
                self.partner_bank_id = self.partner_id.commercial_partner_id.bank_ids.filtered(lambda m: m.bank_default ==True) or self.partner_id.commercial_partner_id.bank_ids[0]
            else:
                self.partner_bank_id = False
        if self.payment_type == 'inbound' and self.invoice_vendor_bill_id:
            partner_ids = [self.invoice_vendor_bill_id[0].company_id.partner_id.id,
                           self.invoice_vendor_bill_id[0].company_id.partner_id.commercial_partner_id.id]
        else:
            partner_ids = [self.partner_id.id, self.partner_id.commercial_partner_id.id]
        return {'domain': {'partner_bank_id': [('partner_id', 'in', partner_ids)]}}
...
class ResPartnerBank(models.Model):
    ...
    @api.onchange('bank_default')
    def _onchange_bank_default(self):
        for rec in self:
            if rec.bank_default:
                bank_account = rec.env['res.partner.bank'].search([('partner_id', '=', rec.partner_id.id)])
                for bank in bank_account:
                    if bank.id != rec.id:
                        bank.write({'bank_default': False})
