...
class MailMessage(models.Model):
    ...
    subject_display = fields.Html(string="Asunto", compute="_compute_subject_display")
    partner_count = fields.Integer(
        string="Recuento de destinatarios", compute="_compute_partner_count"
    )
    record_ref = fields.Reference(
        string="Registro de mensajes",
    ...
    ref_partner_count = fields.Integer(
        string="Seguidores", compute="_compute_ref_partner_count"
    )
    model_name = fields.Char(string="Modelo", compute="_compute_model_name")
    shared_inbox = fields.Boolean(
        string="Bandeja de entrada compartida",
        compute="_compute_dummy",
        help="Utilizado solo para el filtro de bandeja de entrada compartida",
        search="_search_shared_inbox",
    )
    cx_edit_uid = fields.Many2one(string="Editado por", comodel_name="res.users")
    cx_edit_date = fields.Datetime(string="Editado el")
    cx_edit_message = fields.Char(
        string="Editado por", compute="_compute_cx_edit_message"
    )

    active = fields.Boolean(string="Activo", default=True)
    delete_uid = fields.Many2one("res.users", string="Eliminado por")
    delete_date = fields.Datetime(string="Eliminado en")
    deleted_days = fields.Integer(
        string="Días eliminados", compute="_compute_deleted_days"
    )
        ...
        for rec in self:
            ...
            if rec.message_type == "comment":
                ...
                if not rec.author_allowed_id.id == partner_id:
                    raise AccessError(
                        _(
                            "No puedes borrar el siguiente mensaje"
                            "\n\n Asunto: %s \n\n"
                            " Razón: %s"
                            % (rec.subject, _("No eres el autor del mensaje"))
                        )
                    )
                ...
                if not rec.partner_ids:
                    raise AccessError(
                        _(
                            ...
                            "\n\n Asunto: %s \n\n"
                            " Razón: %s"
                            % (rec.subject, _("Destinatarios del mensaje no definidos"))
                        )
                    )
                ...
                if len(rec.partner_ids) > 1:
                    raise AccessError(
                        _(
                            ...
                            "\n\n Asunto: %s \n\n"
                            " Razón: %s"
                            % (rec.subject, _("El mensaje tiene varios destinatarios"))
                        )
                    )
                ...
                if not rec.partner_ids[0].id == partner_id:
                    raise AccessError(
                        _(
                            ...
                            "\n\n Asunto: %s \n\n"
                            " Razón: %s"
                            % (rec.subject, _("No eres el destinatario del mensaje"))
                        )
                    )
